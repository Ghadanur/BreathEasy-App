  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <Card className={cn("shadow-lg hover:shadow-xl transition-shadow duration-300 cursor-pointer flex flex-col justify-between", className)}>
          ...
          <CardContent className="flex flex-col items-center justify-center pt-2 flex-grow">
            {gaugeData && (
              <div className="relative mb-1 transform scale-y-[-1] -rotate-90"> {/* Counter-transform for the gauge container */}
                <SemiCircleGauge {...gaugeData} />
              </div>
            )}
            ...
            {description && !gaugeData && <p className="text-xs text-muted-foreground pt-1">{description}</p>}
          </CardContent>
           {description && gaugeData && (
            <p className="text-xs text-muted-foreground text-center pb-4 px-2">{description}</p>
          )}
        </Card>
      </DialogTrigger>
